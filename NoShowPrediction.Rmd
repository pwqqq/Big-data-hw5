...
nn_fit <- train(V33 ~ ., data = train_df, method = 'nnet', maxit = 1000,linout = 0, preProcess = c('center', 'scale'), trControl = numFolds, tuneGrid=expand.grid(size=c(11,12,13), decay=c(0.1)))
...
results1 <- predict(nn_fit, newdata=data.frame(pred_matrix))
...
## build nn
model_nnet = nnet(V33~.,data = train_df,preProcess = c('center', 'scale'), size = 12, rang = 0.1, decay = 0.1, maxit = 1000)

pred_scaled_pr = data.frame(apply(pred_matrix[1:60000,],2,function(x){(x - mean(x)) / sd(x)}))
pred_scaled_pub = data.frame(apply(pred_matrix[-1:-60000,],2,function(x){(x - mean(x)) / sd(x)}))
