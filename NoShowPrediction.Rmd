...
names = make.names(paste('dim',seq(1,32,length=32)))
...
nn_fit <- train(y ~ ., data = train_df, method = 'nnet', maxit = 1000, linout = 0, 
                ...
                tuneGrid=expand.grid(size=c(13,15,12), decay=c(0.1,0.2)))
...
```
### After parameter tuning, we got size = 13, decay = 0.1
...
## build nn
model_nnet = nnet(y~.,data = train_df,preProcess = c('center', 'scale'), size = 13, rang = 0.1, decay = 0.1, maxit = 1000)
...
#results <- predict(model_nnet, newdata=data.frame(test.x))
#LogLossBinary(y_train[-indexes1],results)
